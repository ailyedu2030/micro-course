...
description: |
  微课管理平台 · 生产环境保护铁律。
  当 AI 在本项目内执行以下任何操作时**强制加载**本 skill:
  1. ssh 连接到 100.74.122.13 (生产服务器)
  2. docker exec / docker logs / docker cp 到生产容器
  3. 修改 /Users/jackie/微课平台/ 下任何已部署到生产的代码
  4. curl/wget 直接请求 microcourse.ailyedu.cn 或其子域名
  5. Playwright 访问 https://microcourse.ailyedu.cn/* (任何路径)
  6. 通过 frp / nginx 转发修改生产配置

  加载本 skill 后,AI 必须遵循铁律并执行 6 步发布流程。
  任何违反铁律的操作 = P0 事故,必须立即停止并回滚。

  触发自检: 每次 AI 准备执行任何 ssh/curl/playwright 操作前,
  必须自问"目标主机/域名是否属于生产?",如果是 → 加载本 skill。
---

# 生产环境保护铁律 (Production Safety)

> **本 skill 是 P0 级硬约束,违反任一条 = 立即停止 + 向用户报告事故。**
...
| GitHub repo `microcourse-admin/...` CI | ❌ 非生产 | 任意操作 |
...
### 案例 1 · 2026-07-01 DatePickerYM 修复事故

**违规**: 直接修改生产后端 jar → kill -HUP 重启 → 在生产 URL Playwright 验证 → 创建 proposal 41-53 真实脏数据 → 反复重启影响真实用户

**正确做法**:
1. local-dev-deploy.sh 跑隔离环境 → Playwright 验证
2. 通过后才走灰度发布

**已修复**: scripts/local-dev-deploy.sh (本次新增)

### 案例 2 · 2026-07-31 PR #161 部署 P0 事故 (INC-2026-07-31-PR161-DEPLOY)

**违规**: heredoc 嵌套 ssh 链式 6 步操作 (scp → docker cp → tar 解压 → mv 替换 dist → nginx reload → 灰度白名单), 任一中间步骤失败都连锁失败. 当 scp 静默失败时, 后续 tar 解压找不到文件, 但 `rm -rf assets && mv newdist/assets` 非原子操作导致生产 `assets/` 目录被删, 用户白屏 30 秒.

**铁律违反**:
- **铁律 3** "一次只做一件事" — 6 步嵌套在一个 atomic command
- **铁律 4** "变更前必须备份" + rollback — 备份存在但 restore 流程不熟练, 加上 nginx reload 失败 (`Address in use`) 折腾 2 次
- **铁律 7** audit trail — 部署步骤散在 heredoc, 失败信息被吞, 无法定位失败步骤

**正确做法** (事故后建立):
1. **禁止 heredoc 嵌套 ssh** — 每个步骤独立 ssh 命令 + 显式 verbose + 每步独立验证 (文件大小 / 文件数 / HTTP 状态 / bundle hash)
2. **禁止 `rm && mv` 非原子操作** — 用 `mv` 原子 rename (`mv /usr/share/nginx/html /usr/share/nginx/html.bak; mv /tmp/newdist /usr/share/nginx/html`)
3. **使用 `scripts/deploy-frontend.sh`** — 11 步独立验证的脚本, 失败立即 `exit 1` 不继续
4. **scp 显式 verbose + 文件大小验证** — 不用判断输出是否"看起来成功", 用 `ls -la` + 大小匹配
5. **本地 main 必须与 origin/main 同步** — 部署前 `git fetch + reset --hard origin/main`

**已修复**:
- `scripts/deploy-frontend.sh` (11 步独立验证)
- `docs/deployment/PR161-DEPLOYMENT-RUNBOOK.md` (部署铁律 + 禁止事项)
- `docs/incidents/2026-07-31-pr161-deployment-p0.md` (完整事故复盘)
- `precheck.sh` 8 项检查 (含部署铁律)
- `docs/console-error-catalog.md` (console 错误诊断目录)

**事故时间线**:
- 20:18 备份生产 dist (246 文件, 6.2MB)
- 20:19-20:21 第一次 build (本地 main 陈旧 → git reset → 重 build)
- 20:21 链式失败: scp 静默 + mv 失败 + assets/ 被删 → 白屏
- 20:21 备份恢复成功 + nginx reload 失败 → reload 第二次 OK
- 20:22 生产完全恢复 (PR #161 部署前状态)
- 20:23-20:36 **改方法重做** (用户3次授权) + 灰度 4 阶段 + 全量
- 20:44 Bug E/F 修复 + 部署
- 20:46 5 分钟监控 0 ERROR
- 21:28 PR #163 squash merge

**事故教训**:
- **每个 ssh 命令独立** + **每步验证** + **mv 原子操作** + **不滥用 heredoc** 是生产部署的 4 大原则
