...
# 2. Browse courses
CCODE=$(curl_code "$BASE/api/courses?page=0&size=5" -H "Authorization: Bearer $TOKEN")
[ "$CCODE" = "200" ] && ok "Browse courses (200)" || fail "Browse courses ($CCODE)"

# 3. Course detail
DETAIL_CODE=$(curl_code "$BASE/api/courses/1" -H "Authorization: Bearer $TOKEN")
[ "$DETAIL_CODE" = "200" ] && ok "Course detail (200)" || fail "Course detail ($DETAIL_CODE)"

# 4. Enrollments
ENR_CODE=$(curl_code "$BASE/api/enrollments" -H "Authorization: Bearer $TOKEN")
...
ORD_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/orders" \
  ...
  -d '{"courseId":1}' 2>/dev/null || echo "500")
...
# 6. Teaching classes
TCH_CODE=$(curl_code "$BASE/api/teaching-classes?page=0&size=5" -H "Authorization: Bearer $TOKEN")
[ "$TCH_CODE" = "200" ] && ok "Teaching classes (200)" || fail "Teaching classes ($TCH_CODE)"

# 7. Grades
GRD_CODE=$(curl_code "$BASE/api/grades" -H "Authorization: Bearer $TOKEN")
...
# 9. Videos
VID_CODE=$(curl_code "$BASE/api/videos?page=0&size=5" -H "Authorization: Bearer $TOKEN")
[ "$VID_CODE" = "200" ] && ok "Videos list (200)" || fail "Videos list ($VID_CODE)"

# 10. Questions
QST_CODE=$(curl_code "$BASE/api/questions?page=0&size=5" -H "Authorization: Bearer $TOKEN")
...
# 12. Reviews
REV_CODE=$(curl_code "$BASE/api/reviews" -H "Authorization: Bearer $TOKEN")
[ "$REV_CODE" = "200" ] && ok "Reviews (200)" || fail "Reviews ($REV_CODE)"

# 13. Notifications
NOT_CODE=$(curl_code "$BASE/api/notifications" -H "Authorization: Bearer $TOKEN")
[ "$NOT_CODE" = "200" ] && ok "Notifications (200)" || fail "Notifications ($NOT_CODE)"

# 14. Admin stats
STA_CODE=$(curl_code "$BASE/api/admin/stats/overview" -H "Authorization: Bearer $TOKEN")
[ "$STA_CODE" = "200" ] && ok "Admin stats (200)" || fail "Admin stats ($STA_CODE)"

# 15. Operation logs
LOG_CODE=$(curl_code "$BASE/api/operation-logs" -H "Authorization: Bearer $TOKEN")
