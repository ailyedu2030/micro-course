...
## 7. 总工程师放行纪律（关联 AGENTS.md §反偏见机制）

本 skill 的使用者**必须遵守 AGENTS.md 中定义的 7 条放行纪律**（位于 AGENTS.md §总工程师放行纪律）：
- **纪律 1**：每条结论必须附证据（禁用"应该没问题"等无证据断言）
- **纪律 2**：每条修复必须做根因分析（禁止表面修复）
- **纪律 7**：任何 `fix` 类型 commit 必须包含【根因】【横向扫描】【防止再发】【验证】四段
- 上述纪律已由 `validate-commit-message.sh` git hook 自动强制执行
