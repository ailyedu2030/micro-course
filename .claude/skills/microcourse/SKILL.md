...
- ❌ 禁止：将 P1-C 标注为 P1-I 而跳过修复（审查必须严格区分客户可感知 vs 内部仅见）
- ❌ 禁止：只做 1-2 个维度的审查（必须 4 维全覆盖）
- ❌ 禁止：用自己的眼睛逐文件检查（必须用 Agent Team 并行审查）
- ✅ 每条审查结果必须标注 `P0 / P1-C / P1-I / P2` 分类，不标注视为无效
...
输出: 逐项 PASS/FAIL + 文件:行号 + P0/P1-C/P1-I/P2 分类
- P0 = 数据安全/核心功能不可用/客户首次操作必现错误
- P1-C = 客户可感知的不一致、错误消息、体验降级
- P1-I = 内部仅见的代码/文档问题，正常使用不可感知
- P2 = 代码整洁、安全加固建议、性能优化
```
