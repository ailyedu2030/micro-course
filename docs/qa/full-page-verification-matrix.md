...
| B16 | TeachingClassList | 新增教学班（排课时间段校验） | 🟡 | 创建被校验拦截，完整创建待补 |
...
| B20 | EnrollmentList `/enrollments` | 列表/筛选/导出 | 🟡 | 仅加载 |
| B21 | FavoriteList `/favorites` | 列表/筛选 | 🟡 | 仅加载 |
...
| C3 | ChapterList `/chapters` | 选课/章节列表/新增 | 🟡 | 仅加载 |
| C4 | VideoList `/videos` | 新增/上传/编辑/封面/删除 | ✅ | 上传验证，编辑删除待补 |
| C5 | QuestionList `/questions` | 新增（选项/答案）/编辑/删除/预览/导入导出 | ✅ | 增删改验证，预览/导入待补 |
| C6 | QuestionPreview | 预览渲染（4 题型） | ⬜ | 待验证 |
| C7 | ExerciseList `/exercises` | 新增/编辑/删除/选题 | ✅ | 增改验证，删除待补 |
| C8 | ExerciseForm `/courses/:id/exercises/form` | 独立表单页 | 🟡 | 弹窗版验证，独立页待补 |
| C9 | DiscussionDetail `/discussions/:id` | 管理端通过/驳回/删除/回复管理 | 🟡 | 仅加载，交互待补 |
...
| # | 页面/路由 | 核心验证项 | 状态 | 备注 |
|---|----------|-----------|------|------|
| D1 | TeacherDashboard | 统计/收益/待办/通知 | 🟡 | 仅加载 |
...
| D8 | TeacherOfflineList | 列表/筛选 | 🟡 | 仅加载 |
| D9 | TeacherOfflineSessions | 新增场次/编辑/签到 | ✅ | 新增校验验证 |
| D10 | TeacherSlideOverview | 上传/筛选/状态 | ✅ | 上传验证 |
| D11 | SlideManage `/teacher/courses/:id/slides/manage` | 课件工作台/页面编辑/音频 | 🟡 | 仅加载 |
...
| D17 | MyProposals | 列表/编辑/预览/Word/PDF/删除 | 🟡 | 列表验证，导出待补 |
| D18 | MicroSpecialtyInvites | 待处理/归档/接受/取消 | 🟡 | 仅加载 |
...
| E20 | AchievementWall | 徽章解锁条件 | 🟡 | 仅加载 |
| E21 | WeeklyReport | 周报数据 | 🟡 | 仅加载 |
...
| # | 页面/路由 | 核心验证项 | 状态 | 备注 |
|---|----------|-----------|------|------|
| F1 | AcademicDashboard | 统计/预警/热门课程 | 🟡 | 仅加载 |
| F2 | LearningAnalytics `/academic/stats` | 院系数据/预警 | 🟡 | 仅加载 |
| F3 | EnrollmentOverview `/academic/enrollments` | 选课总览/筛选 | 🟡 | 仅加载 |
| F4 | MicroSpecialtyReview | 微专业审核通过/驳回 | 🟡 | 仅加载 |
| F5 | MicroSpecialtyProposalReview | 申报批准/驳回 | ✅ | 批准验证 |
| F6 | MicroSpecialtyFeaturedReview | 置顶申请审批 | ⬜ | 待验证 |
| F7 | MicroSpecialtyCrossDeptReview | 跨学院教师审核 | ⬜ | 待验证 |
| F8 | MicroSpecialtyClassImport | 班级导入 | ⬜ | 待验证 |
| F9 | MicroSpecialtyGoldManage | 金标设置 | ⬜ | 待验证 |
| F10 | StorageApplicationReview | 存储申请表审批 | ⬜ | 待验证 |
...
**交互待补（⬜）**：C6 QuestionPreview、E11 StudentOfflineSession、F6 置顶审核、F7 跨学院审核、F8 班级导入、F9 金标管理、F10 存储审批

**仅加载待深化（🟡）**：B16 教学班完整创建、B20/B21 管理列表、C3 ChapterList、C8 ExerciseForm 独立页、C9 DiscussionDetail 管理交互、D1 教师看板、D8 线下列表、D11 SlideManage、D17 MyProposals 导出、D18 邀请接受、E3 我的课程进度、E6 训练中心、E10 SlidePlayer、E20 成就、E21 周报、F1-F4 教务页
