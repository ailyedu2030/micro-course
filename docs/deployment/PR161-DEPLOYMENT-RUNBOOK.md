...
## 🚨 部署铁律 (INC-2026-07-31-PR161-DEPLOY 事故后新增)

按 P0 事故复盘 ([docs/incidents/2026-07-31-pr161-deployment-p0.md](../../incidents/2026-07-31-pr161-deployment-p0.md))，新增以下部署铁律：

### 🚫 禁止 1：heredoc 嵌套 ssh + 嵌套 docker exec

```bash
# ❌ 错误: 一个 ssh 命令做 6 步, 任何中间失败都连锁失败 (P0 事故根因)
ssh ubuntu@100.74.122.13 << 'EOF'
  docker cp /tmp/admin-pr161.tar.gz admin-1:/tmp/
  docker exec admin-1 tar xzf /tmp/admin-pr161.tar.gz
  docker exec admin-1 mv /usr/share/nginx/html /tmp/old
  docker exec admin-1 mv /tmp/newdist /usr/share/nginx/html
  docker exec admin-1 nginx -s reload
EOF
```

```bash
# ✅ 正确: 每个步骤独立 ssh 命令, 独立验证
scp -v /tmp/admin-pr161.tar.gz ubuntu@100.74.122.13:/tmp/
ssh ubuntu@100.74.122.13 "ls -la /tmp/admin-pr161.tar.gz"  # 验证传输成功
ssh ubuntu@100.74.122.13 "docker cp /tmp/admin-pr161.tar.gz admin-1:/tmp/"
ssh ubuntu@100.74.122.13 "docker exec admin-1 ls -la /tmp/admin-pr161.tar.gz"  # 验证
ssh ubuntu@100.74.122.13 "docker exec admin-1 tar xzf /tmp/admin-pr161.tar.gz"
# ...每步独立验证...
```
...
```bash
# ❌ 错误: 失败时旧目录已删, 新目录未就位 → 白屏 (P0 事故触发)
docker exec admin-1 sh -c "
  rm -rf /usr/share/nginx/html/assets && \
  mv /tmp/newdist/assets /usr/share/nginx/html/assets
"
```

```bash
# ✅ 正确: mv 是原子 rename, 失败时旧目录仍在原位
docker exec admin-1 sh -c "
...
### 🚫 禁止 3：本地 main 落后 origin/main 时 build

```bash
# ❌ 错误: 本地 main HEAD = dd242620 (R14), origin/main = d729d36e (PR #162)
# build 出的 dist 不含 PR #161 修复 → 部署后用户看到的还是旧版本
cd micro-course-admin && npm run build
scp dist/.../admin.tar.gz server:/tmp/
```

```bash
# ✅ 正确: 部署前必须先 sync main
git fetch origin main
git reset --hard origin/main  # 强制本地 main 同步到 origin/main
git log --oneline origin/main -5  # 确认要部署的 commit 在 origin/main
cd micro-course-admin && rm -rf dist && npm run build
tar czf /tmp/admin-pr161.tar.gz -C dist .
scp -v /tmp/admin-pr161.tar.gz server:/tmp/
```

### ✅ 推荐：使用 `scripts/deploy-frontend.sh` 一键安全部署

```bash
# 部署前置:
cd micro-course-admin && npm ci && npm run build
tar czf /tmp/admin-pr161.tar.gz -C dist .
bash scripts/deploy-gate.sh check  # gate 必须 open

# 一键部署 (内置 11 步独立验证):
bash scripts/deploy-frontend.sh /tmp/admin-pr161.tar.gz
```

`scripts/deploy-frontend.sh` 内置：
- 11 个独立 ssh 命令
- 每步独立验证 (scp Exit 0 / 文件大小匹配 / HTTP 200 / bundle hash 匹配)
- 用 mv 原子操作
- 失败立即 `exit 1` 不继续
...
**Updated**: 2026-07-31 (INC-2026-07-31-PR161-DEPLOY 事故后加部署铁律章节)