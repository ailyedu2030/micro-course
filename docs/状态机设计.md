...
> 版本：v1.6（2026-08-07 新增 §10 课件状态机：课件创建/替换/删除 + 音频 GENERATING/READY/FAILED）
> 日期：2026-06-11
> 状态：正式发布
>
...
| `PENDING_REVIEW` (1) | 待审核 | 已提交，等待管理员审核 |
...
#### T8: REJECTED → PENDING_REVIEW（重新提交）
...
> **实现状态**：✅ **复用课程状态机**（`DRAFT/PENDING_REVIEW/APPROVED/REJECTED` 子集）+ 审核日志表
...
3. 教师修改课程内容
4. 教师点击"重新提交"
5. 课程状态从 `REJECTED` → `PENDING_REVIEW`
...
> **概述**：HTML 课时使用 `course_slides.content_type` 字段区分渲染来源。视频/PPT 课件的 `content_type` 为 `PPT_RENDERED`（由 PPT 渲染引擎生成），而 HTML 直接上传课件的 `content_type` 为 `HTML_DIRECT`。

#### 7.6.1 状态定义

| 状态值 | 名称 | 业务含义 |
|--------|------|---------|
| `PPT_RENDERED` | PPT 渲染 | 课件由 PPT/PPTX 上传后渲染生成（默认值） |
| `HTML_DIRECT` | HTML 直传 | 课件由 HTML 文件直接上传，无需渲染引擎处理 |

#### 7.6.2 状态转换规则

| 属性 | 说明 |
|------|------|
| **转换路径** | `PPT_RENDERED → HTML_DIRECT`（上传 HTML 时自动设置） |
| **逆转换** | 不支持（HTML_DIRECT 不会自动转回 PPT_RENDERED） |
| **触发操作** | `POST /api/courses/{courseId}/slides/upload` 上传 `.html` / `.htm` 文件 |
| **状态影响** | **不改变 `course_slides.status`**，仅影响 `course_slides.content_type` 字段 |
| **渲染分支** | 播放器根据 `contentType=HTML_DIRECT` 选择 HTML 渲染器而非 PPT 翻页器 |
| **幂等性** | 幂等，同课时重复上传 HTML 只覆盖 content_type+文件，不产生新 slide 行 |

#### 7.6.3 渲染路径对比

| 维度 | PPT_RENDERED | HTML_DIRECT |
|------|-------------|------------|
| 源文件 | PPTX（ZIP 格式） | HTML/HTM（文本格式） |
| 渲染引擎 | PPT 翻页渲染器 + Canvas 截图 | 浏览器内嵌 iframe sandbox |
| 文件大小限制 | ≤ 50MB | ≤ 5MB |
| 状态流转 | UPLOADING → TRANSCODING → READY/FAILED | 无流转（上传即就绪） |
| 音频注入 | 页面级 TTS + 分段音频 | HTML template 内 AUDIO_SEG_NN_URL 占位符替换 |

#### 7.6.4 冲突检测

| 场景 | 处理策略 |
|------|---------|
| PPT 课件的课时重新上传 HTML | `content_type` 从 `PPT_RENDERED` 更新为 `HTML_DIRECT`，旧渲染数据清理 |
| HTML 课件的课时重新上传 PPT | 同上，`content_type` 更新为 `PPT_RENDERED`，走 PPT 渲染流程 |
| 同时上传多媒体文件到同一课时 | 按 `(courseId, chapterId, sectionId)` UPSERT，最后上传者获胜 |
...
> **实现状态**：🟢 **完整实现**（v1.4 同步：`pay()` / `paymentCallback()` / `cancelOrder()` / `refund()` 全部接入 `canTransitionTo` 白名单；`OrderStatusTest` 待补）
> **实现里程碑**：Round 6-3（`enums/OrderStatus.java` 枚举 + `canTransitionTo` 白名单 + `pay()` 接入 + `V65` 乐观锁兜底）+ v1.4 同步（`paymentCallback` 改用 `canTransitionTo(PAID)`、`cancelOrder` 改用 `canTransitionTo(CANCELLED)`、`refund` 服务方法已实现）
> **枚举位置**：`enums/OrderStatus.java`（`String` 值，含 `canTransitionTo` 白名单 + `fromValue` 容错解析；范式同 `EnrollmentStatus`，零迁移兼容存量字符串）
> **服务层校验**：`OrderServiceImpl.pay()` / `paymentCallback()`（`OrderStatus.fromValue` → `canTransitionTo(...)` → `ErrorCode.INVALID_STATUS_TRANSITION`），并与下方 `WHERE status='PENDING'` CAS 乐观锁形成「业务语义 + 并发竞态」双重防御
...
> ✅ **v1.4 同步状态**：
> - `cancelOrder()`（L309–313）：已改用 `OrderStatus.fromValue(status).canTransitionTo(CANCELLED)` 白名单
> - `refund()`（L328+）：已实现，含进度检查(<10%) + 防重入(ConcurrentHashMap) + 套餐退选(`unenrollBundleCourses`)
> - `paymentCallback()`（L475–488）：已改用 `OrderStatus.fromValue(order.getStatus()).canTransitionTo(PAID)` 白名单
> - ⚠️ `OrderStatusTest` / `OrderStatusMachineTest` 待补（迁移至未来 Phase 7 backlog）
...
| `REFUNDED` | 已退款 | 退款完成（终态；v1.4 同步：`refund()` 已实现，含进度检查/防重入/套餐退选） |
...
|------|------|
| **触发角色** | 学生 / 管理员 |
| **Pre-condition** | `OrderStatus.fromValue(status).canTransitionTo(CANCELLED)`（v1.4 已接入枚举白名单） |
| **Action** | `WHERE status='PENDING'` CAS 置 `CANCELLED` |
| **Post-condition** | 仅待支付订单可取消；CAS 失败抛"订单状态已变更" |

#### T4: PAID → REFUNDED（v1.4 同步：已实现）

| 属性 | 说明 |
|------|------|
| **触发角色** | 学生（用户申请退款）/ 管理员 |
| **Pre-condition** | `OrderStatus.fromValue(status).canTransitionTo(REFUNDED)`（枚举白名单） + 学习进度 < 10% + 非重入（ConcurrentHashMap） |
| **Action** | `refund()` 服务方法（OrderServiceImpl L328+）：置 `REFUNDED` + 退款流水 + 套餐退选(`unenrollBundleCourses`) + 通知 |
| **Post-condition** | 订单进入已退款终态；套餐关联课程自动退选；CAS 失败或重入抛"订单状态已变更" / "退款处理中" |
...
## 10. 课件状态机（V300-V310 交互式课件 v2 + legacy course_slides）

> **承载表**：`course_slides`（legacy 元数据，status 0/1/2/3）+ `slide_ppt_pages` / `slide_html_units`（v2 单元，V300/V303）+ `slide_ppt_page_audios` / `slide_html_segment_audios`（V302/V305 音频 status GENERATING/READY/FAILED）。
> **说明**：v2 课件生命周期以「课件单元是否存在」表达 EMPTY/READY 语义（无显式 status 列），与 legacy `course_slides.status` 双轨并行；删除走 `DELETE /slides/courseware`（v1+v2 全量）。

### 10.1 课件创建（EMPTY → PPT_RENDERING → PPT_READY / HTML_UPLOADED → HTML_READY）

```
EMPTY（无课件单元）
   │  POST /slides/upload（.pptx，multipart）
   ▼
PPT_RENDERING（course_slides.status=0，POI 渲染中）
   │  后台渲染完成（双写 slide_ppt_pages）
   ▼
PPT_READY（course_slides.status=2；slide_ppt_pages 行存在）

EMPTY（无课件单元）
   │  POST /slides/upload（.html/.htm，HtmlSanitizer 消毒）
   ▼
HTML_UPLOADED（course_slides.status=2；content_type=HTML_DIRECT；slide_pages 行存在）
   │  POST /html/sections/{id}/unit（createUnit/createUnitFresh 初始化 v2 单元）
   ▼
HTML_READY（slide_html_units 行存在 + html_sanitized 可入播放器）
```

**守卫**：
- PPT 渲染失败：`course_slides.status=3` + `error_message`（如"文件保存失败"）；不生成 slide_ppt_pages → 可重传覆盖。
- HTML 消毒后内容为空（HtmlSanitizer 全移除）→ 拒绝上传（16012 HTML_SANITIZE_REMOVED_ALL）。
- 重复上传（file_hash 命中）→ 幂等返回既有课件，不重复渲染（uk_slides_course UNIQUE）。

### 10.2 课件替换（READY → RENDERING → READY，保留 audio 元数据）

```
PPT_READY / HTML_READY
   │  POST /slides/upload（同 section/chapter 覆盖上传）
   ▼
RENDERING（course_slides.status=0，新文件渲染中；旧 slide_ppt_pages 行保留）
   │  渲染完成（先清旧行再双写新行：renderSync 语义）
   ▼
PPT_READY（新版页面；音频元数据保留关联：脚本/音频按 pptPageId 版本延续）
```

**守卫**：替换期间播放器仍可读旧版（旧行未删）；渲染成功后才清旧行（避免替换失败丢数据）。

### 10.3 课件删除（READY → DELETED）

```
PPT_READY / HTML_READY
   │  DELETE /api/courses/{courseId}/slides/courseware?sectionId=&chapterId=
   ▼
DELETED（幂等；v1 + v2 全量清理）
```

**清理范围**（SlideServiceImpl.deleteCourseware）：
- v2：`slide_ppt_pages` + `slide_ppt_page_scripts` + `slide_ppt_page_audios`（CASCADE）+ `slide_html_units` + `slide_html_segment_scripts` + `slide_html_segment_audios` + `slide_ppt_flow`（section 级规则）
- legacy：`course_slides` + `slide_pages`（按 sectionId/chapterId 全量）
- **权限**：TEACHER/ADMIN + verifyAccess（课程 owner 校验，P1-I-17 补音频文件清理见 F-2026-08-07 系列）
- 幂等：目标不存在时仍返回 200（R.ok()），重复删除无副作用

### 10.4 音频状态机（GENERATING → READY / FAILED，含 PENDING 边界）

```
PENDING（无脚本/无音频；narration_status=PENDING）
   │  POST /scripts/{scriptId}/audios（生成音频）或批量 generate
   ▼
GENERATING（V302/V305 status=GENERATING；TtsWorkerService 轮询消费）
   ├── TTS 成功 → READY（audio_token/audio_duration_ms/file_size_bytes/storage_path/completed_at 落库）
   └── TTS 失败 → FAILED（error_message；可重试重新生成）
```

**边界与幂等**：
- 仅消费 `status=GENERATING` 行；成功后置 READY 不再轮询（TtsWorkerService 幂等）。
- READY 后重新生成 → 新版本行（每页/每段按 active script 版本延续），旧行保留（历史版本）。
- narration_status 聚合（v_slide_ppt_page_status / v_slide_html_unit_status）：任一 active script 有 READY 音频 → AUDIO_READY；有脚本无音频 → AUDIO_GENERATING；无脚本 → PENDING。
- 流式 GET `/courseware/audio/{token}`：status=READY 才 200 流式；非 READY 返 202（"Audio not ready yet"）。

### 10.5 状态值速查

| 承载 | 值 | 语义 |
|------|----|----|
| course_slides.status | 0 | 处理中/渲染中（RENDERING） |
| course_slides.status | 2 | 就绪（PPT/HTML 可播放） |
| course_slides.status | 3 | 失败（error_message 透传） |
| slide_ppt_page_audios.status / slide_html_segment_audios.status | GENERATING | TTS 生成中 |
| 同上 | READY | 生成完成可播放 |
| 同上 | FAILED | 生成失败（可重试） |
| narration_status（聚合） | PENDING / AUDIO_GENERATING / AUDIO_READY | 页/单元级讲述状态 |
...
| 8 | 订单 `orders.status` | 🟢 完整（v1.4） | `enums/OrderStatus`（String） | ✅ 全接入 | `pay()`/`paymentCallback()`/`cancelOrder()`/`refund()` 全部走 `canTransitionTo` 白名单 | ❌ 无专门（backlog） | `V65`（version，实由 `V57` 首建） | Round 6-3 + v1.4 |
| — | 通知 `notifications.type` | N/A | `enums/NotificationType`（类型枚举，非状态机） | — | — | `NotificationTriggerTest` | — | P0-7 |

**关键诚实修正（避免假修复声称）**：
- **课程 / 视频** 两者为 **🟡 部分实现**：课程与视频**无 `canTransitionTo` 声明式白名单**（视频以 CAS 并发门控替代）。
- **订单** v1.4 同步后为 **🟢 完整实现**：`pay()` / `paymentCallback()` / `cancelOrder()` / `refund()` 全部接入 `canTransitionTo` 白名单（详见 §8）。
- **订单状态机测试待补**（`OrderStatusTest` / `OrderStatusMachineTest`），纳入 backlog 不阻塞合并。
...
| v1.2 | 2026-07-07 | 课程管理域 drift 修复 (course-domain-drift-fix)：补全 DRAFT→PENDING_REVIEW 第 5 项守卫「至少一个视频或练习/课件」 (S1)；补全 PENDING_REVIEW→REJECTED 第 3 项守卫「reason ≥ 10 字符」 (S2)；补全 CLOSED→PUBLISHED 守卫「lastPublishedAt != null」 (S3)；新增通用端点拒绝约定 (S4)：PUT /api/courses/{id}/status 拒绝 status=1/4；约定所有状态变更通过 CourseStateMachine 单例入口 | 总工程师 |
| v1.3 | 2026-07-07 | 用户管理域 drift 修复 (user-domain-drift-fix)：禁用 INACTIVE→DELETED 超额转换 (§1.3.1)；新增 INACTIVE→ACTIVE 激活守卫三选一条件(emailVerified/casBound/adminForceActivate) (§1.3.2)；新增 DELETED→[*] 180天自动物理删除约定 (UserRetentionCleanupJob @Scheduled) (§1.3.3)；UserStatus 枚举 4×4 转换矩阵完整定义 | 总工程师 |
| v1.4 | 2026-07-07 | 订单支付域文档同步 (order-domain-drift-fix)：同步 §8 诚实说明——cancelOrder() 已改用 canTransitionTo 白名单；refund() 已实现完整服务方法含进度检查/防重入/套餐退选；paymentCallback() L475 改用 canTransitionTo(PAID)；数据字典 orders.version 补充登记，orders/payments.status NOT NULL 修正 | 总工程师 |
| v1.5 | 2026-07-27 | HTML 课时内容类型说明：新增 §7.6 HTML 课时内容类型 (contentType=HTML_DIRECT)，阐明 PPT_RENDERED → HTML_DIRECT 不改变 slide.status 仅影响 contentType 字段 | AI |
| v1.6 | 2026-08-07 | 新增 §10 课件状态机（P1-I-7）：课件创建 EMPTY→PPT_RENDERING→PPT_READY / HTML_UPLOADED→HTML_READY、替换 READY→RENDERING→READY（保留 audio 元数据）、删除 READY→DELETED（v1+v2 全量）、音频 GENERATING→READY/FAILED（含 PENDING 边界）+ 状态值速查表 | AI |
