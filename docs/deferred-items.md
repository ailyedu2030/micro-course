...
| **D19-2** | **P1-I** | **6 个 dependabot MAJOR 升级** (#192 vue-i18n 9→11 / #159 pinia 2→4 / #157 happy-dom 15→20 / #155 springdoc 2.5→3.1 / #154 easyexcel 3.3→4.0 / #153 openpdf 1.3→3.0) | 4/6 已升级: PR #280 (happy-dom) / #281 (easyexcel) / #282 (pinia) / #283 (vue-i18n). 2/6 暂缓: #155 springdoc 3.1 与 SB 3.3.x 的 `EndpointCondition must be used on @Bean methods` API breaking change (2026-08-20 D7-1 实施失败验证: 升级 SB 3.2.12→3.3.13 + springdoc 2.5.0→3.1.0 后 117 个测试错误, 已诚实回滚), 真正根因是 springdoc 3.x 与 `spring-boot-starter-actuator` 的 `EndpointCondition` API 冲突, 不仅 SB 版本限制, 需配套改造 actuator 注解 (@Endpoint/@ReadOperation/@WriteOperation) 才能升级; #153 openpdf 3 移除 PdfPageEvent 等 lowagie API (项目 CertificateServiceImpl + StorageApplicationPdfGenerator 需代码迁移) + AGPL 许可证风险 | 4 项已闭环 + 2 项明确暂缓 (springdoc 真实根因已升级) | D19-2 67% 完成. 后续 2 项: springdoc 升级专题 (SB 3.4+ + actuator 注解改造 + 117 测试回归) / openpdf 等代码迁移 + 许可证评估专题 |
...
当前候选发布已完成代码、测试、CI 和本地隔离部署闭环，教师模块功能问题已收口。经本轮评估，当前允许延期但不阻断发布准备的项目仅保留以下 2 项：
...
以下事项不进入 deferred-items，必须在正式放行前完成：

1. `bash scripts/deploy-gate.sh check`
2. `ROLLBACK_PLAN.md` 与当前候选发布保持一致
3. `CHANGELOG.md` 补齐当前候选发布说明
4. staging / production gray 策略明确
5. 项目负责人授权生产动作

### 当前状态

- `deploy-gate.sh check`：已执行，门禁窗口有效
- `ROLLBACK_PLAN.md`：已补 Phase 6 候选发布回滚说明
- `CHANGELOG.md`：已补 `Unreleased` 当前候选发布记录
- 正式生产放行：**尚未授权，当前仍停留在发布准备阶段**
...
## v1.21.0 (2026-07-04) 课程管理全量修复后再评估

### 新登记 (P2/P3 范围,待后续 sprint)

| # | 来源 | 问题 | 现状 | 计划目标版本 |
|---|------|------|------|-------------|
| 12 | 全量扫描 | `views/student/Checkout.vue:107-108` - "多课程串行下单"非事务原子性,中间失败前面订单已扣款不退 | ✅ 已修复 — `POST /api/orders/batch` @Transactional 批量接口 + 前端优先调用 | v1.22.0 (2026-07-04 已修) |
| 13 | 全量扫描 | `views/student/MyCourses.vue:692-695` - completedVideos 是 `章节数 × 进度` 近似值,非精确视频完成数 | ✅ 已修复 — LearningProgressVO 加 completedVideos 字段,后端精确计算 | v1.22.0 (2026-07-04 已修) |
| 14 | 全量扫描 | `views/courses/CourseList.vue:357, 585` - teacherName 搜索参数被注释,后端尚未支持 | ✅ 已修复 — CoursePageQuery 加 teacherName + 前端解除注释 | v1.22.0 (2026-07-04 已修) |
| 15 | 全量扫描 | `views/admin/Dashboard.vue:374` - FIXME 举报处理功能缺失 | ✅ 已修复 — V142 migration + ReportController + ReportsManagement.vue 全链路 | v1.22.0 (2026-07-04 已修) |
...
- ✅ P0-1 课程发布越权 (CourseAdminServiceImpl.updateStatus/submitForReview 加 isOwnerOrAdmin + CAS)
- ✅ P0-2 正确率趋势基于逐题 isCorrect 而非整卷 passed
- ✅ P0-3 错题本多章节 Map<Long, List<Long>>
- ✅ P0-4 待批改用 needsManualGrading 布尔列替代 JSON LIKE
- ✅ P0-5 考试路由 404 + 章节完成度联动
- 14 项 P1-C + 18 项 P1-I + 12 项 P2 全部修复
- 新增 5 个回归测试 (18 用例) + 3 个 Flyway migration + 2 个工具类

---

*Last updated: 2026-07-04 — Phase J final review*
