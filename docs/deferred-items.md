...
| **F10-D3** | **P1-I** | **i18n 存量硬编码中文 4757 处（2026-08-13 部署时实测）**：i18n CI 门禁已接入（advisory 拦截增量），但存量 93 文件未迁移，en-US 切换仍显示中文 | 当前默认中文用户正常使用，客户不可感知；en-US 用户文案未国际化 | 放行（P1-I，已接入增量门禁），不阻断 | 按 Top 文件分批迁移（里程碑 1: Top 10 文件全部迁移后，CI 切换为硬阻断）。已迁移：菜单 102 + PlatformShareConfig 89 + 空状态 2 |
