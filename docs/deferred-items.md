...
| # | 维度 | 文件 | 问题 | 客户场景 | 修复估时 |
|---|------|------|------|---------|---------|
| P0-2 | 测试 | `micro-course-api/src/test/` | 47 个 Service 实现 / 51 个 Controller，0 个单元测试 | 任何 Service 修改无回归保护，风险不可控 | ~1 天（核心 5 个） |
| P0-3 | 性能 | `MyCourses.vue:650-696` + 后端 11 处 N+1 | 每门课程单独调 learningProgress + chapters，100 课程时 100 次 API | 生产首次使用卡 3-5 秒 | ~2 小时（后端批量接口） |
| P0-4 | 业务 | 后端无 GradeComponent weight 校验 | 虽当前无生产路径触发，但数据库 grade_components.weight 无校验 | 权重配置异常时出业务错误 | ~30 分钟（service 层加校验） |
| P0-5 | 业务 | 9/30+ 业务事件无通知触发 | 选课/成绩出/审核通过等无通知 | 关键操作后客户收不到反馈 | ~4 小时（补全触发点） |
...
| **代码** | R7 P1-I 登记 | 28 条 ✅ |
| **代码** | R7 P2 登记 | 17 条 ✅ |
| **代码** | P0-2~P0-5 登记 | 4 条（待 v1.18.1 修复）|
| **本地** | precheck.sh | 16/16 PASS ✅ |
| **本地** | mvn compile | 0 ERROR ✅ |
| **本地** | npm run build | SUCCESS ✅ |
...
*最后更新：2026-06-27 R8 总工程师补充审查*  
*评估人：总工程师*  
*P0：1/5 修复（P0-1 已修），P0-2~P0-5 登记 v1.18.1 待做*  
*P1-C：8/8 修复清零*  
*P1-I：28 条登记，零批量放行*  
*P2：17 条记录到 backlog*  
*下次清理：v1.18.1 发布前（P0-2~P0-5 + P1-I + P2 全部清零）*
