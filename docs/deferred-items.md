...
| # | 来源 | 问题 | 现状 | 计划目标版本 |
|---|------|------|------|-------------|
| 12 | 全量扫描 | `views/student/Checkout.vue:107-108` - "多课程串行下单"非事务原子性,中间失败前面订单已扣款不退 | 业务影响:客户操作 10000+ 次约 1 次触发 | v1.22.0 引入批量下单接口 |
| 13 | 全量扫描 | `views/student/MyCourses.vue:692-695` - completedVideos 是 `章节数 × 进度` 近似值,非精确视频完成数 | 客户在前端看到的进度可能与实际视频进度有偏差 | v1.22.0 后端 `getLearningProgress()` 增加 completedVideos 字段 |
| 14 | 全量扫描 | `views/courses/CourseList.vue:357, 585` - teacherName 搜索参数被注释,后端尚未支持 | 管理员无法按教师姓名快速筛课程,只能按 teacherId | v1.22.0 后端 CoursePageQuery 加 teacherName 字段 |
| 15 | 全量扫描 | `views/admin/Dashboard.vue:374` - FIXME 举报处理功能缺失 | 客户在前端无法处理举报内容 | v1.23.0 新功能 review_report 表 + 举报处理工作流 |
