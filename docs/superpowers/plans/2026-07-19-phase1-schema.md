...
- [ ] **Step 3: 本地 isolated 环境跑 migration 验证**

```bash
cd /Users/jackie/微课平台
bash scripts/local-dev-deploy.sh --skip-build 2>&1 | tail -20
```

Expected: 
- 看到 "✅ V202 migration applied" 或 Flyway 启动日志显示 V202 OK
- 看到 "✅ 15 通过, ❌ 0 失败"
...
```bash
cd /Users/jackie/微课平台
bash scripts/local-dev-deploy.sh --skip-build 2>&1 | tail -15
```
...
```bash
cd /Users/jackie/微课平台
bash scripts/local-dev-deploy.sh --skip-build 2>&1 | tail -10
```
...
- [ ] **Step 1: 完整跑 local-dev-deploy 验证**

```bash
cd /Users/jackie/微课平台
bash scripts/local-dev-deploy.sh 2>&1 | tail -25
```

Expected:
- 看到所有 9 个新 migration 依次 applied
- 看到 "✅ 15 通过, ❌ 0 失败"
- 看到 "🔓 生产门禁已自动打开"
